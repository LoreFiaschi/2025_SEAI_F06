...
def augment_state_and_targets(data: List[Tuple[torch.Tensor, float, float]]):
    augmented = []
    for st, z_w, z_b in data:
        # identity
        augmented.append((st, z_w, z_b))
        # rotate 180
        st_r = torch.rot90(st, 2, [1,2])
        augmented.append((st_r, z_w, z_b))
        # hflip
        st_h = torch.flip(st, [2])
        augmented.append((st_h, z_w, z_b))
        # vflip
        st_v = torch.flip(st, [1])
        augmented.append((st_v, z_w, z_b))
    return augmented
...
def self_play_game(mcts_white, mcts_black):
    ...
    return augment_state_and_targets(list(reversed(data)))
