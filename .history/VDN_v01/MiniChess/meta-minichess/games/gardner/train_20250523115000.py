...
def self_play_game(mcts, value_net, buffer, max_buffer_size=5000):
    board = chess.Board()
    states = []
    rewards_w = []
    rewards_b = []
    ...
    while not board.is_game_over():
        state = board.fen()
        states.append(encode_state_as_tensor(state).detach())  # ensure no graph

        # Applichi l'MCTS per selezionare la mossa migliore
        move = mcts.search(board, value_net)
        board.push(move)

        # Reward shaping
        r_w = material_count(board, chess.WHITE)
        r_b = material_count(board, chess.BLACK)
    ...
    z_w_total, z_b_total = z_w, z_b
    for i in reversed(range(len(states))):
        z_w_total += rewards_w[i]
        z_b_total += rewards_b[i]
        buffer.append((states[i], z_w_total, z_b_total))
