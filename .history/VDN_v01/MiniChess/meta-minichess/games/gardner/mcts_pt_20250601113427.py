...
class MCTS:
    ...
    def _enqueue(self, leaf: _Node) -> None: #prepara batch di stati da valutare  
    ...
    def _flush(self) -> None:
         if not self._queue:
             return
         # separa foglie terminali da non‐terminali
         terminals, nonterms = [], []
         ...
         # 1) backpropago subito i terminali con reward esatto
         for node in terminals:
             # result() restituisce ±1 o REWARD_DRAW
             node.backpropagate(node.state.result())

         # 2) per i non‐terminali chiedo alla rete
         if nonterms:
             states = torch.stack([encode_state_as_tensor(n.state) for n in nonterms], dim=0).to(DEVICE)
             with torch.no_grad():
                 raw = self.value_net(states)
             for node, out in zip(nonterms, raw):
                 if out.numel() == 1:
                     v = out.item()
                 else:
