...
def self_play_game(mcts_white: MCTS, mcts_black: MCTS) -> List[Tuple[torch.Tensor, float, float]]:
    ...
    while not state.is_terminal():
        ...
        move = mcts.search(state, temperature=1)
...
def self_play_wrapper(args):
    ...
    mcts_b = MCTS(net_b, iterations_MCTS, rng=random.Random(seed + 10000))
...
if __name__ == "__main__":
    ...
    for cycle in range(1, num_cycles + 1):
        ...
        seeds = [cycle * 100 + i for i in range(games_per_cycle * 2)]
        with Pool(processes=2) as pool:
            results = pool.map(self_play_wrapper, [
                (current_net, best_net, s) if i % 2 == 0 else (best_net, current_net, s)
                for i, s in enumerate(seeds)
            ])
        buffer = [item for sublist in results for item in sublist]
