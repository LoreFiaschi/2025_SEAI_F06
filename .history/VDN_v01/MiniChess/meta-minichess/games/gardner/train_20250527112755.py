...
# Definizioni integrate per evitare circular import
class ChessValueDataset(Dataset):
    """
    Dataset contenente tuple (state_tensor, [r_w, r_b])
    """
    def __init__(self, data: List[Tuple[torch.Tensor, float, float]]):
        self.data = data
    ...
    def __getitem__(self, idx: int):
        state, r_w, r_b = self.data[idx]
        target = torch.tensor([r_w, r_b], dtype=torch.float32)
        return state, target
...
def train_value_net(
    ...
    """
    Allena la rete net usando train_loader con MSELoss e AdamW.
    """
    scaler = GradScaler()
...
def play_det(
    ...
    """
    Gioca una partita deterministica (tau=0) tra due reti e restituisce il risultato.
    """
...
def compute_material_potential(board: np.ndarray) -> float:
    ...
    somma dei valori assoluti dei pezzi da PIECE_VALUES, con segno per il colore.
    """
    total = 0.0
...
def arena(
    ...
    wins = draws = 0
    mcts_curr = MCTS(current_net, iterations_MCTS, rng=random.Random(seed or BASE_SEED))
    mcts_best = MCTS(best_net, iterations_MCTS, rng=random.Random((seed or BASE_SEED) + 1))
    half = games // 2
