...
def arena(current_net: torch.nn.Module, best_net: torch.nn.Module, games: int = arena_games) -> float:
    wins = draws = enemy_wins = 0
    mcts_curr = MCTS(current_net, iterations_MCTS, rng=random.Random(42))
    mcts_best = MCTS(best_net, iterations_MCTS, rng=random.Random(43))
    half = games // 2
    ...
    logger.info(f"Arena: wins_white={wins_white}, wins_black={wins - wins_white}, draws={draws}, enemy_wins={enemy_wins}")
